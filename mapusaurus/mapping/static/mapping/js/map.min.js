...
function makeDots(features) {
    for (var points = [], i = 0; i < features.length; i++) {
        var feat = features[i], poly = feat.geometry.coordinates[0], reverseBounds = L.latLngBounds(poly);
        if (reverseBounds) for (var j = 0, len = feat.properties.volume / 10 >> 0; len > j; j++) {
...
function initDotDensity() {
    map._initPathRoot();
    var svg = d3.select("#map").select("svg"), g = svg.append("g").attr("class", "leaflet-zoom-hide");
    return function(data) {
        var points = makeDots(data.tracts.features).map(function(point) {
            var latlng = new L.LatLng(point[1], point[0]);
            return map.latLngToLayerPoint(latlng);
        });
        console.log(points), g.selectAll("circle").data(points).enter().append("circle").attr("class", "densityDot").attr({
            cx: function(d) {
                return d.x;
            },
            cy: function(d) {
