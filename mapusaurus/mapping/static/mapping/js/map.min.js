...
var dotDensity = function() {
    function init() {
        map._initPathRoot(), svg = d3.select("#map").select("svg"), g = svg.append("g").attr("class", "leaflet-zoom-hide"), 
        $.when($.ajax({
            url: "/api/msa",
            data: {
                metro: 41860
    ...
    function draw() {
        var data = sfMSA, points = makeDots(data.tracts.features).map(function(point) {
...
window.console || (console = {
    ...
    $(".tabs").show(), $(window).resize(function() {
        setMapHeight();
    }), dotDensity.init(), map.on("zoomend", dotDensity.draw), map.on("zoomstart", dotDensity.wipe), 
